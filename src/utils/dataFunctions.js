...
export const filterDataByDateRange = (
  ...
  data.forEach((item) => {
    ...
    data.forEach((endeks) => {
      const indexDate = new Date(endeks.maxDemandSifirlamaTarihi);
      if (
        indexDate >= startDateObj &&
        indexDate <= endDateObj &&
        indexDate !== null
      ) {
...
export const pageHeader = (url) => {
  switch (url) {
    ...
    case "/all-read-indexes/:id":
      return "Sayac Endeks Detay Sayfasi";
...
export const mapToSeparateArray = (data) => {
  const separateArray = [];
